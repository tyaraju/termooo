          ...
          return <Row className='justify-content-md-center'>
            <div key={kb.level} className={`kb${kb.level} justify-content-md-center d-flex flex-row`}>
              {kb.keyboardLetters.map((letter, index) => {
                return <Col className='col-md-auto d-flex'><button className={
                  ( letter !== '<-' && letter !== 'ENTER') ? 'btn' : ( letter === '<-') ? 'btn backspace' : 'btn enter'
                } key={index}>{letter}</button></Col>
              })}
            </div>
          </Row>
        })}
